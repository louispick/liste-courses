...
import { Heart, Loader2, RefreshCw, HeartCrack, Sparkles, Hourglass, ArrowLeft, X, Check, Eye, Trophy, Lock, Calendar } from 'lucide-react';
...
export default function Quiz() {
  ...
  // Bornes de la session actuelle
  const sessionStartIndex = (currentSession - 1) * BATCH_SIZE;
  const sessionEndIndex = currentSession * BATCH_SIZE;

  if (gameState && gameState.answers) {
      const userKey = user.email.replace(/\./g, '_');
      
      // 1. Calcul des points (UNIQUEMENT POUR LA SESSION EN COURS)
      // On parcourt les questions de la session
      ...
      // 2. Calcul de l'avancement GLOBAL (pour savoir où on en est)
      ...
      if (isFirstLoadRef.current) {
          ...
          if (myProgress > lastSeenIndex) {
              // On checke seulement sur la session en cours
              let heartsBefore = 0;
              for (let i = sessionStartIndex; i < sessionEndIndex; i++) {
                  if (i >= lastSeenIndex) continue; // On ne compte que ce qu'on a déjà vu
              ...
              // Note: Ce calcul est approximatif sur le changement de session, mais ok pour MVP
  ...
  useEffect(() => {
      ...
      // On force l'index à être dans la session
      if (currentQIndex < sessionStartIndex) {
          setCurrentQIndex(sessionStartIndex);
      }
      
      // Avancement auto si on a déjà répondu (reprise de partie)
      if (myProgress > currentQIndex && myProgress < sessionEndIndex) {
  ...
  const handleBack = () => {
      if (step === 'partner') {
          setStep('self');
          setMyChoice(null);
      } else if (currentQIndex > sessionStartIndex) { // Ne pas reculer hors session
  ...
  const handleStartNextSession = async () => {
      ...
      // Le state se mettra à jour via Firestore et le useEffect repositionnera l'index
  };

  if (loading) return <div className="flex justify-center pt-20"><Loader2 className="animate-spin text-sun-yellow" /></div>;

  // ÉTATS DE SESSION
  const isSessionFinishedMe = myProgress >= sessionEndIndex;
  const isSessionFinishedPartner = partnerProgress >= sessionEndIndex;
  const diff = myProgress - partnerProgress;

  // Numéro de la question DANS la session (1 à 20)
  ...
  return (
    <div className="pt-4 pb-24 px-4 max-w-sm mx-auto h-[80vh] flex flex-col relative">
      ...
      {/* MODALES ET EFFETS (Code identique) */}
      {showCelebration && (
          <div className="absolute inset-0 z-50 flex items-center justify-center pointer-events-none">
              <div className="animate-float-up">
                  <Heart className="w-32 h-32 text-red-500 fill-red-500 drop-shadow-2xl" />
                  <div className="text-center mt-2 font-black text-white text-xl bg-red-500 px-4 py-1 rounded-full shadow-lg">MATCH !</div>
              </div>
          </div>
      )}
      {showFailure && (
          <div className="absolute inset-0 z-50 flex items-center justify-center pointer-events-none">
              <div className="animate-float-up">
                  ...
                  <div className="text-center mt-2 font-black text-white text-xl bg-gray-400 px-4 py-1 rounded-full shadow-lg">OUPS...</div>
              </div>
          </div>
      )}
      {showNewHearts && (
          <div className="absolute inset-0 z-50 flex items-center justify-center bg-white/90 backdrop-blur-sm animate-in fade-in duration-500">
              <div className="bg-white rounded-3xl p-8 soft-shadow text-center border-4 border-sun-yellow transform animate-in zoom-in duration-300">
                  ...
                  <div className="text-6xl font-black text-red-500 mb-2 drop-shadow-sm">+{newHearts}</div>
                  <p className="text-xl font-bold text-red-400 mb-8">Nouveaux Cœurs !</p>
                  <button onClick={closeNewHearts} className="w-full bg-deep-blue text-white font-bold py-3 rounded-xl">Continuer</button>
              </div>
          </div>
      )}

      {/* HISTORIQUE MODAL (AVEC ONGLETS) */}
      {showHistory && (
          <div className="absolute inset-0 z-50 bg-white rounded-3xl soft-shadow flex flex-col animate-in slide-in-from-bottom-10 duration-300">
              <div className="p-4 border-b border-gray-100 flex justify-between items-center">
                  <h2 className="font-bold text-deep-blue text-lg">Historique</h2>
              ...
              <div className="flex p-2 gap-2 border-b border-gray-100 bg-gray-50">
                  <button 
                    onClick={() => setHistoryTab('answers')}
                    className={clsx("flex-1 py-2 rounded-xl text-sm font-bold transition-colors", historyTab === 'answers' ? "bg-white text-deep-blue shadow-sm" : "text-gray-400 hover:text-gray-600")}
                  >
                      Réponses
                  </button>
                  <button 
                    onClick={() => setHistoryTab('sessions')}
                    className={clsx("flex-1 py-2 rounded-xl text-sm font-bold transition-colors", historyTab === 'sessions' ? "bg-white text-deep-blue shadow-sm" : "text-gray-400 hover:text-gray-600")}
                  >
                      Sessions
                  </button>
              </div>
              ...
              <div className="flex-1 overflow-y-auto p-4 space-y-4">
                  {historyTab === 'sessions' ? (
                      <div className="space-y-3">
                          {(gameState.sessionHistory || []).length === 0 && (
                              <div className="text-center text-gray-400 py-10">Pas encore de session terminée.</div>
                          )}
                          {[...(gameState.sessionHistory || [])].reverse().map((session, idx) => (
                              <div key={idx} className="bg-white border-2 border-gray-100 rounded-2xl p-4 flex justify-between items-center">
                                  <div>
                                      <h3 className="font-bold text-deep-blue">Session {session.id}</h3>
                                      <p className="text-xs text-gray-400">{new Date(session.date).toLocaleDateString()}</p>
                                  </div>
                                  <div className="flex items-center gap-4">
                                      <div className="flex flex-col items-center">
                                          <Heart className="w-5 h-5 text-red-500 fill-red-500" />
                                          <span className="font-bold">{session.hearts}</span>
                                      </div>
                                      <div className="flex flex-col items-center opacity-50">
                                          <HeartCrack className="w-5 h-5 text-gray-400" />
                                          <span className="font-bold">{session.errors}</span>
                                      </div>
                                  </div>
                              </div>
                          ))}
                      ...
                      // Liste des réponses (inchangée, mais limitée à ce qu'on a vu)
                      QUESTIONS.slice(0, Math.max(myProgress, partnerProgress)).map((q, idx) => {
                          // ... (Code d'affichage des réponses idem précédent)
                              ...
                              <div key={q.id} className={clsx("border-2 rounded-xl p-3 text-sm", status === 'success' ? "border-green-200 bg-green-50" : status === 'fail' ? "border-red-200 bg-red-50" : "border-gray-100 bg-gray-50")}>
                                  ...
                                  <div className="grid grid-cols-2 gap-2 text-xs">
                                      <div className="bg-white/60 p-2 rounded-lg">
                                          ...
                                          {myAns ? <><p>Choix : <strong>{myAns.self}</strong></p><p className="text-gray-500">Pense que {partnerName} : {myAns.partner}</p></> : <span className="italic text-gray-400">...</span>}
                                      </div>
                                      <div className="bg-white/60 p-2 rounded-lg">
                                          <p className="font-bold text-deep-blue mb-1">{partnerName}</p>
                                          {partnerAns ? (status === 'waiting_me' ? <span className="italic text-gray-400">Caché</span> : <><p>Choix : <strong>{partnerAns.self}</strong></p><p className="text-gray-500">Pense que {myName} : {partnerAns.partner}</p></>) : <span className="italic text-gray-400">...</span>}
                                      </div>
                                  </div>
                              </div>
                          );
      ...
      >
          {(currentQIndex > sessionStartIndex || step === 'partner') && !isSessionFinishedMe && (
      ...
      <div className="flex-1 flex flex-col justify-center">
          ...
          {/* CAS 1 : SESSION TERMINÉE POUR TOUS LES DEUX -> BILAN */}
              ...
              <div className="bg-white rounded-3xl p-8 soft-shadow text-center animate-in zoom-in duration-300">
                  ...
                  <div className="flex justify-center gap-8 mb-8">
                      <div className="text-center">
                          <div className="text-4xl font-black text-red-500">{totalHearts}</div>
                          <div className="text-xs text-gray-400 uppercase font-bold mt-1">Matchs</div>
                      </div>
                      <div className="text-center">
                          <div className="text-4xl font-black text-gray-400">{totalErrors}</div>
                          <div className="text-xs text-gray-400 uppercase font-bold mt-1">Ratés</div>
                      </div>
                  </div>

                  <button 
                      onClick={handleStartNextSession}
                      className="w-full bg-deep-blue text-white font-bold py-4 rounded-xl shadow-lg shadow-deep-blue/20 hover:scale-105 transition-transform flex items-center justify-center gap-2"
                  >
                      <Sparkles className="w-5 h-5" />
                      Lancer la Session {currentSession + 1}
                  </button>
              </div>
          ) : isSessionFinishedMe ? (
              // CAS 2 : FINI POUR MOI, ATTENTE PARTENAIRE
              <div className="bg-white rounded-3xl p-8 soft-shadow text-center opacity-80">
                  <div className="bg-gray-100 p-4 rounded-full w-20 h-20 mx-auto mb-6 flex items-center justify-center animate-pulse">
                  ...
                  <p className="text-gray-500 mb-6">
                      Attends que {partnerName} termine ses {sessionEndIndex - partnerProgress} dernières questions pour voir le bilan.
                  </p>
                  <div className="w-full bg-gray-100 rounded-full h-2 mb-2">
                      <div className="bg-sun-yellow h-2 rounded-full transition-all duration-1000" style={{ width: `${(partnerProgress % BATCH_SIZE) / BATCH_SIZE * 100}%` }}></div>
                  </div>
                  <p className="text-xs text-gray-400 text-right">{partnerProgress % BATCH_SIZE} / {BATCH_SIZE}</p>
              </div>
          ) : (
              // CAS 3 : JEU EN COURS (CARTE STANDARD)
      ...
      <div className="mt-8 text-center">
          <button onClick={resetGame} className="text-[10px] text-gray-300 hover:text-red-400 flex items-center justify-center gap-1 mx-auto">
              <RefreshCw className="w-3 h-3" /> Réinitialiser la partie
          </button>
      </div>
    </div>
  );
...
function ChoiceBtn({ onClick, label, selected, variant = 'self' }) {
    ...
    return (
        <button onClick={onClick} className={clsx(baseClass, variant === 'self' ? selfClass : partnerClass)}>
            {label}
        </button>
    );
}
