...
export default function Quiz() {
  ...
  // Helper pour récupérer l'historique des sessions (compatible Array/Object)
  const getSessionHistory = () => {
      if (!gameState?.sessionHistory) return [];
      if (Array.isArray(gameState.sessionHistory)) return gameState.sessionHistory; // Legacy support
      return Object.values(gameState.sessionHistory); // New Map support
  };
  ...
  if (gameState && gameState.answers) {
      ...
      if (isFirstLoadRef.current) {
          ...
          if (myProgress > lastSeenIndex) {
              let heartsBefore = 0;
              // On checke sur la session en cours ou la précédente si on vient de finir
              for (let i = sessionStartIndex; i < sessionEndIndex; i++) {
  ...
  useEffect(() => {
      if (!gameState) return;
      if (currentQIndex < sessionStartIndex) setCurrentQIndex(sessionStartIndex);
      if (myProgress > currentQIndex && myProgress < sessionEndIndex) setCurrentQIndex(myProgress);
  ...
  const handleBack = () => {
      ...
      else if (currentQIndex > sessionStartIndex) { setCurrentQIndex(prev => prev - 1); setStep('self'); setMyChoice(null); }
  };
  ...
  // Filtrage pour l'historique des réponses : Seulement la session courante (ou celle qu'on vient de finir)
  // On limite l'affichage entre sessionStartIndex et (sessionEndIndex ou progression actuelle)
  const historyStart = sessionStartIndex;
  const historyEnd = Math.min(Math.max(myProgress, partnerProgress), sessionEndIndex);
  
  // Si on est au tout début d'une session, on n'affiche rien (plutôt que la session d'avant)
  // sauf si l'utilisateur veut voir les sessions précédentes via l'onglet Sessions
  const currentSessionQuestions = QUESTIONS.slice(historyStart, historyEnd);
  ...
  return (
    <div className="pt-4 pb-24 px-4 max-w-sm mx-auto h-[80vh] flex flex-col relative">
      ...
      {showHistory && (
          <div className="absolute inset-0 z-50 bg-white rounded-3xl soft-shadow flex flex-col animate-in slide-in-from-bottom-10 duration-300">
              <div className="p-4 border-b border-gray-100 flex justify-between items-center">
                  ...
                  <button onClick={() => setShowHistory(false)} className="p-2 bg-gray-100 rounded-full hover:bg-gray-200"><X className="w-5 h-5 text-gray-500" /></button>
              </div>
              
              <div className="flex p-2 gap-2 border-b border-gray-100 bg-gray-50">
                  <button onClick={() => setHistoryTab('answers')} className={clsx("flex-1 py-2 rounded-xl text-sm font-bold transition-colors", historyTab === 'answers' ? "bg-white text-deep-blue shadow-sm" : "text-gray-400 hover:text-gray-600")}>Réponses (Session {currentSession})</button>
                  <button onClick={() => setHistoryTab('sessions')} className={clsx("flex-1 py-2 rounded-xl text-sm font-bold transition-colors", historyTab === 'sessions' ? "bg-white text-deep-blue shadow-sm" : "text-gray-400 hover:text-gray-600")}>Sessions passées</button>
              </div>
              ...
              <div className="flex-1 overflow-y-auto p-4 space-y-4">
                  {historyTab === 'sessions' ? (
                      ...
                      // LISTE RÉPONSES (FILTRÉE SESSION COURANTE)
                      currentSessionQuestions.length === 0 ? (
                          <div className="text-center text-gray-400 py-10">Aucune réponse dans cette session. Jouez !</div>
                      ) : (
                              ...
                              return (
                                  <div key={q.id} className={clsx("border-2 rounded-xl p-3 text-sm", status === 'success' ? "border-green-200 bg-green-50" : status === 'fail' ? "border-red-200 bg-red-50" : "border-gray-100 bg-gray-50")}>
                                      <div className="flex justify-between items-start mb-2">
                                          <span className="font-bold text-gray-700">Q{sessionStartIndex + idx + 1}. {q.text}</span>
      ...
      <div onClick={() => setShowHistory(true)} className="flex justify-between items-center mb-6 bg-white p-3 rounded-2xl soft-shadow relative cursor-pointer active:scale-[0.98] transition-transform hover:bg-gray-50">
      ...
      <div className="flex-1 flex flex-col justify-center">
          {isSessionFinishedMe && isSessionFinishedPartner ? (
              ...
              <div className="bg-white rounded-3xl p-8 soft-shadow text-center opacity-80">
                  ...
                  <p className="text-gray-500 mb-6">Attends que {partnerName} termine ses {sessionEndIndex - partnerProgress} dernières questions.</p>
